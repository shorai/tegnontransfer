...
public class UserHasSites {
    ...
    static final String loadSQL = "select *"// + fields
            + "from users_has_site "
            + "order by TegnonUsers_id,Sites_SiteID";
...
// NB DateTimeStamp is a reserved word in SQL 92  MS SQL should NEVER allow it tio be used as a column name
    static final String insertSql = "insert into users_has_site("
            + fields
            + ") values(?,?)";
    static PreparedStatement insertStatement = null;
/*  This is a pure relationship
    static final String updateSql = "update users_has_site set"
            + " UserName= ?"
   ...
   // Integer id;
    ...
    void transfer(Connection in, Connection out) {
       ...
       //     updateStatement = out.prepareStatement(updateSql);
            ...
            logger.severe("Transfer User_has_Sites  Failed  after processing  " + count 
                    + " records, inserts = " + numInserts 
                    + " Updates="+numUpdates 
                    + "   Exception:" + sexc.getLocalizedMessage());
            System.out.println("Transfer User_has_Sites  Failed  after processing  " + count 
                    + " records, inserts = " + numInserts 
                    + " Updates="+numUpdates 
                    + "   Exception:" + sexc.getLocalizedMessage());
